        ...
        stage('Build Docker Image') {
            steps {
                script {
                    ...
                    sh "cd /home/elliot/meusite/"
                    sh "docker build -t meusite:latest ."
                    sh "docker tag meusite:latest meusite:latest"
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    sshagent([SSH_CREDENTIALS_ID]) {
                        ...
                        docker pull meuste:latest &&
                        docker run -dit -p 80:80 --restart always -v meusite:/usr/local/apache2/htdocs/ kawansilva/meusite
                        "
                        """
                    }
                }
